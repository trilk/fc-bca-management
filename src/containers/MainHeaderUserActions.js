  ...
  return (
    <CDropdown inNav className="c-header-nav-item mx-2">
      ...
      <CDropdownMenu placement="bottom-end" className="pt-0">
        ...
        {!sysUser.isAdmin && (
          <>
            <CDropdownItem onClick={onOpenModal}>
              <CIcon name="cil-star" className="mr-2 text-success" /> Bói nhà vô
              địch
            </CDropdownItem>
            <CDropdownItem to={`/event-user/${sysUser.id}`}>
              <CIcon name="cil-user" className="mr-2 text-danger" /> Dự đoán của
              tao
            </CDropdownItem>
          </>
        )}
        {sysUser.isAdmin && (
          <CDropdownItem onClick={onOpenAdminModal}>
            <CIcon name="cil-star" className="mr-2 text-success" /> Quản lý dữ
            liệu
          </CDropdownItem>
        )}
        <CDropdownDivider />
        {sysUser.isAdmin && (
          <CDropdownItem to={`/events/today`}>
            <CIcon name="cil-zoom-in" className="mr-2 text-info" /> Soi kèo hôm
            nay
          </CDropdownItem>
        )}
        <CDropdownItem to={`/events/${event.id}`}>
          <CIcon name="cil-sort-numeric-down" className="mr-2 text-primary" />{" "}
          Bảng xếp hạng
        </CDropdownItem>
      </CDropdownMenu>
    </CDropdown>
