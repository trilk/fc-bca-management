...
import {
    CModalHeader, CModal, CModalBody, CModalFooter, CButton, CRow, CCol, CInput, CAlert
} from '@coreui/react'
...
const ModalTeamSelection = props => {
    ...
    const onSubmit = (confirmed) => {
        if (confirmed) {
            fbDb.BettingService.userBetWinnerTeam(eventId, sysUser.id, selectedTeam.id).then((response) => {
    ...
    if (show && myBet && myBet.canEdit) {
        evtTeams.forEach((item, idx) => {
            // push column
            columns.push(
                <div className="team-cotainer col-6 col-md-3 py-2 px-3" key={idx} onClick={() => onSelectTeam(item)}>
                    <div className={`team-info ${selectedTeam && item.id === selectedTeam.id ? 'selected' : ''}`}>
            ...
            if ((idx + 1) % 4 === 0) { columns.push(<div className="w-100" key={'_' + idx}></div>) }
        })
    }
    // setColumns(cols)
    ...
    return (
        <>
            <CModal className=""
                ...
                size="lg" color="info" scrollable={true} className="ts-modal">
                        ...
                        {selectedTeam &&
                            <div className="d-flex justify-content-center">
                                <div className="fav-team d-flex align-items-center px-4">
                                    <CIcon width="48" name={selectedTeam.flagCode} className="mr-3" ></CIcon>
                                    <span>{selectedTeam.name} VÔ ĐỊCH</span>
                                </div>
                            </div>}
                        {!(selectedTeam && myBet.canEdit) &&
                            <CAlert color={myBet.color} className="">
                                <CIcon name={myBet.icon} size="xl" className="mr-2 mb-1"></CIcon>
                ...
                <CModalFooter className="d-flex justify-content-center">
                    {myBet && myBet.canEdit && <CButton color="primary" onClick={() => onSubmit(true)} disabled={!selectedTeam}>
