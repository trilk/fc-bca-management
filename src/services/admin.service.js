...
class AdminService {
  startBettingRound = async (eventId, round) => {
    ...
    const evtUsersRef = await firebase.db
      .collection(`${COLLECTION.EVENT}/${eventId}/${COLLECTION.USER}/`)
      .where("active", "==", true)
      .get();
    ...
    _.each(evtUsers, async (user) => {
      var ubRef = firebase.db
        .collection(ubCollection)
        .doc(`R${round}_${user.id}`);
    ...
    return batch.commit().then(() => {
      return {
        status: "OK",
        message: `Tao xong vong du doan cho ${evtUsers.length} nguoi choi!`,
      };
    });
  };
  ...
  finalBettingRound = async (eventId, round) => {
    ...
    const ubCollection = _.toLower(eventId) + COLLECTION.USER_BET;
    const betResult = await firebase.db
      .collection(ubCollection)
      .where("round", "==", round)
      .get();
    ...
    const evtSummaryRef = firebase.db
  ...
  setUserBetResult = async (data) => {
    ...
    const usersResult = await Promise.all(
      ubRef.docs.map(async (ub) => {
        let betValues = !_.isEmpty(ub.data().matches[data.gameId])
          ? ub.data().matches[data.gameId]
          : {
              bet: 0,
              result: -1,
            };
        ...
        if (data.round > 3) {
          betValues["point"] = calculatePoint(
            betValues.bet,
            data.result,
            data.round,
            betValues.usedStar || false,
            data.isOver,
            betValues.isOver
          );
        }
    ...
    return batch
      ...
      .then(() => {
        return {
          status: "OK",
          message: "Tỉ số trận đấu đã được cập nhật thành công.",
        };
      })
      .catch((err) => {
        console.error(err);
