...
class ExpenseService {
  ...
  getPayments = async (eventId, group, evtGroups) => {
    ...
    if(group === GROUP.ALL) {
      paymentRefs = firebase.db.collection(pmCollection).where('payBy', '==', group)
    } else {
  ...
  getPaidList = async (eventId, group, evtGroups) => {
    const pmCollection = _.toLower(eventId) + COLLECTION.PAYMENT
    let paymentRefs = await firebase.db.collection(pmCollection)
      .where('payBy', '==', group).get();

    return paymentRefs.docs.map((doc) => {
      const grp = _.find(evtGroups, ['id', doc.data().payTo])
      return {
        id: doc.id,
        name: doc.data().name,
        date: doc.data().date,
        payTo: doc.data().payTo,
        payToName: _.isEmpty(grp) ? Service_Provider : grp.name,
  ...
  getRecievedList = async (eventId, group, evtGroups) => {
    const pmCollection = _.toLower(eventId) + COLLECTION.PAYMENT
    let paymentRefs = await firebase.db.collection(pmCollection)
      .where('payTo', '==', group).get();

    return paymentRefs.docs.map((doc) => {
      const grp = _.find(evtGroups, ['id', doc.data().payBy])
      return {
        id: doc.id,
        name: doc.data().name,
        date: doc.data().date,
        payBy: doc.data().payBy,
        payByName: _.isEmpty(grp) ? Service_Provider : grp.name,
        payFor: doc.data().payFor,
      }
    })
  }
