...
const EventSummary = (props) => {
  ...
  const fields = [
    ...
    {
      key: "winner",
      label: largeScreen ? "Vô địch" : "VĐ",
      _classes: `text-center ${panelDisplay ? "d-none" : ""}`,
    },
    {
      key: "corrected",
      label: largeScreen && !panelDisplay ? "Đúng" : "Đ.",
      _classes: "align-middle text-center",
    },
    {
      key: "wrong",
      label: largeScreen && !panelDisplay ? "Sai" : "S.",
      _classes: "text-center",
    },
    {
      key: "missed",
      label: largeScreen && !panelDisplay ? "Bỏ qua" : "BQ",
      _classes: `text-center d-none ${panelDisplay ? "" : " d-sm-table-cell"}`,
    },
    { key: "percent", label: "%", _classes: `text-center` },
    {
      key: "point",
      label: largeScreen && !panelDisplay ? "Điểm" : "Pt",
      _classes: "text-center",
    },
  ];
  ...
  const onSelectRound = (rNo, data) => {
    ...
    const tmpTable = orderBy(
      usPts,
      [`${field}.point`, `${field}.corrected`, "name"],
      ["desc", "desc", "asc"]
    );
  ...
  useEffect(async () => {
    if (isEmpty(userPoints) && evtTeams.length > 0 && storedUsers.length > 0) {
      ...
      let userPts =
        eventSummary.winnerTeam !== ""
          ? []
          : await fbDb.BettingService.getUserBettingRound(
              sysUser.group,
              eventId,
              event.round
            );
      ...
      fbDb.BettingService.calculatePointTable(
        userPts,
        eventSummary,
        evtTeams,
        storedUsers,
        ...
        const rNo = eventSummary.winnerTeam !== "" ? 0 : event.round;
        onSelectRound(rNo, response);
      });
    }
          ...
          <CRow>
            ...
            <CCol>
              <CNav className="justify-content-end">
                <CNavLink
                  active={selectRound === 1}
                  onClick={() => onSelectRound(1)}
                >
                  {largeScreen ? "Vòng 1" : "V1"}
                </CNavLink>
                {event.round >= 2 && (
                  <CNavLink
                    active={selectRound === 2}
                    onClick={() => onSelectRound(2)}
                  >
                    {largeScreen ? "Vòng 2" : "V2"}
                  </CNavLink>
                )}
                {event.round >= 3 && (
                  <CNavLink
                    active={selectRound === 3}
                    onClick={() => onSelectRound(3)}
                  >
                    {largeScreen ? "Vòng 3" : "V3"}
                  </CNavLink>
                )}
                {event.round >= 4 && (
                  <CNavLink
                    active={selectRound === 4}
                    onClick={() => onSelectRound(4)}
                  >
                    {largeScreen ? "Vòng 4" : "V4"}
                  </CNavLink>
                )}
                {event.round == 5 && (
                  <CNavLink
                    active={selectRound === 5}
                    onClick={() => onSelectRound(5)}
                  >
                    {largeScreen ? "Vòng 5" : "V5"}
                  </CNavLink>
                )}
                {event.round >= 2 && (
                  <CNavLink
                    active={selectRound === 0}
              ...
              winner: (item) => (
                <td
                  className={`text-center ${
                    panelDisplay ? "d-none" : ""
                  } ${isOutTeam(item.favTeam)}`}
                >
                  <CIcon
                    className=""
                    width="36"
                    name={item.favTeam?.flagCode || "flag-tbd"}
                  ></CIcon>
                </td>
              ),
              corrected: (item) => (
              ...
              missed: (item) => (
                <td
                  className={`text-center align-middle d-none ${
                    panelDisplay ? "" : "d-sm-table-cell"
                  }`}
                >
                  {selectRound === 0
                    ? item.result.missed
                    : item[`R${selectRound}`].missed}
                </td>
              ),
              percent: (item) => (
                <td className={`text-center align-middle`}>
                  {Math.round(
                    (selectRound === 0
                      ? item.result.percent
                      : item[`R${selectRound}`].percent) * 100
                  )}
                  %
                </td>
              ),
