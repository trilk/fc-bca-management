            ...
            let userGames = await fbDb.BettingService.getCurrentBettingGames(sysUser.group, event.id, event.round, '', []).then(async (response) => {
                if (response !== null) {
                    ...
                    await response.games.forEach((game, idx) => {
                        ...
                        const field = {
                            key: game.id,
                            label: largeScreen ? (game.firstTeam + ' - ' + game.secondTeam) : (game.firstTeam.slice(0, 3) + ' ' + game.secondTeam.slice(0, 3)),
        ...
        <>
            <CRow className="event-data-header px-3 mb-1">
                <CCol className={`title pl-0 ${largeScreen ? '' : 'mobile'}`}>
                    <CIcon name="cil-zoom-in" className="mr-2 mb-1" size="xl"></CIcon><span>SOI KÈO CỦA BẠN</span>
                </CCol>
            </CRow>
