...
const UserBetHistory = (props) => {
  ...
  const fields = [
    ...
    {
      key: "result",
      label: largeScreen ? "Kết Quả" : "KQ",
      _classes: "align-middle text-center",
    },
    {
      key: "point",
      label: largeScreen ? "Điểm" : "Pts",
    ...
    const fetchData = async () => {
      if (_.isEmpty(userData) && evtTeams.length > 0) {
        ...
        setEventSummary({
          ...
          winnerTeam: "",
          userFavorTeam: eventSummary.users[userId]
            ? eventSummary.users[userId].betTeam
            : "",
        });
        await fbDb.BettingService.getGamesBetByUser(
          event.id,
          event.round,
          userId
        ).then(async (response) => {
      ...
      <CRow className="event-data">
        <CCol md="4" className="pr-2">
          <CCard className="mb-3">
            <CCardHeader>
              {isEmpty(evtSummary.userFavorTeam)
                ? "Chưa chọn đội lên ngôi"
                : "Chọn " + evtSummary.userFavorTeam + " vô địch!"}
            </CCardHeader>
            <CCardBody>
              <CRow>
                ...
                <CCol className="justify-content-center d-flex">
                  <CIcon
                    name={
                      isEmpty(evtSummary.userFavorTeam)
                        ? "flagTbd"
                        : getFlagCode(1, evtSummary.userFavorTeam, "", 1)
                    }
                    width="100"
                  ></CIcon>
                </CCol>
          ...
          <CCard>
            ...
            <CCardBody className="py-2 px-0">
              <CChartPie
                datasets={[
                  {
                    backgroundColor: ["#2eb85c", "#e55353", "#ced2d8"],
                    data: [
                      userData.betSum.corrected,
                      userData.betSum.wrong,
                      userData.betSum.missed,
                    ],
                  },
                ]}
        ...
        <CCol xl="8" className="pl-2">
          <CCard className="user-stat mb-3">
            <CCardBody>
              <CRow>
                <CCol md="4" className="name d-flex align-items-center">
                ...
                <CCol className="d-flex align-items-center justify-content-center">
                  <div className="text-center">
                    <span className="text-danger">{userData.betSum.wrong}</span>
                    <p>Sai</p>
                  </div>
                </CCol>
                <CCol className="d-flex align-items-center justify-content-center">
          ...
          {userData.rounds.map((round, idx) => (
            <CCard className="bet-round" key={idx}>
              <CCardHeader
                onClick={() => setSelectRound(idx + 1)}
                className={idx < userData.rounds.length - 1 ? "finished" : ""}
                title={`Xem lại lựa chọn vòng ${idx + 1}`}
              >
                <CRow>
                  ...
                  <CCol className="d-flex align-items-center justify-content-center">
                    <div className="text-center">
                      <span className="text-success">
                        {round.betSum.corrected}
                      </span>
                    </div>
                  </CCol>
                  <CCol className="d-flex align-items-center justify-content-center">
                    <div className="text-center">
                      <span className="text-danger">{round.betSum.wrong}</span>
                    </div>
                  </CCol>
                  <CCol className="d-flex align-items-center justify-content-center">
                    <div className="text-center">
                  ...
                  <CCol className="d-flex align-items-center justify-content-center">
                    <div className="text-center">
                      <span className="text-success">{round.betSum.point}</span>
                    </div>
                  </CCol>
                  <CCol className="d-flex align-items-center justify-content-center">
                    <div className="text-center">
                    ...
                    scopedSlots={{
                      name: (item) => (
                        <td className="align-middle">
                          <span>
                            {item.firstTeam}{" "}
                            {item.goals.first90 + ":" + item.goals.second90}{" "}
                            {item.secondTeam}
                          </span>
                        </td>
                      ),
                      bet: (item) => (
                        <td className={`align-middle text-center`}>
                          ...
                          <span className="position-relative">
                            ...
                            {item.isOver !== undefined && (
                              <CIcon
                                style={{ bottom: "-10px" }}
                                name={`cil-arrow-circle-${
                                  item.isOver ? "top" : "bottom"
                                }`}
